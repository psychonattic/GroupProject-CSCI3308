...
class GameBoard:
	...
	def run(self): #main game loop - currently just has a quit button
		...
		endTurnSurf = pygame.font.Font(None, 40).render('End Turn', True, BLACK, GREEN) 
		...
		if(self.endturn):
			...
			self.DISPLAY.blit(avFundsSurf, avFundsRect) #displays available money
			...
			if event.type == MOUSEBUTTONDOWN: #checks for mouse click
				...
				if rollRect.collidepoint((mousex, mousey)) and not self.endturn:
					...
					if self.d1 != 0 and self.d2 != 0: #if button has been pressed dice appear
						self.diceDisplay(self.d1, self.d2)
	...
	def diceDisplay(self, d1, d2):
		...
		self.DISPLAY.blit(die1, (self.boardsize/6, 4*(self.boardsize/6))) #displays d1
		self.DISPLAY.blit(die2, (self.boardsize/1.5, 4*(self.boardsize/6))) #displays d2
